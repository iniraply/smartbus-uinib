// src/pages/LoginDriver.jsx (FINAL CLEAN + TOASTIFY)
...
function LoginDriver() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/api/auth/login/driver", formData);
      ...
      // Notifikasi Sukses dengan Nama Driver (jika ada)
      const namaDriver = res.data.user.nama || "Driver";
      toast.success(`Selamat datang, ${namaDriver}! Siap bertugas?`);

      // Arahkan ke halaman Jadwal (Dashboard Driver)
    ...
    <div className="flex justify-center items-center min-h-screen bg-brand-cream font-sans">
      <div className="w-full max-w-md p-8 space-y-6 bg-white shadow-lg rounded-xl animate-fade-in-down border-t-4 border-brand-primary">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 font-medium text-gray-500">Login Driver</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          {/* Tombol Kembali (Opsional, agar tidak terjebak) */}
